...
macro_rules! generate_tests {
    ($($name:ident),* $(,)?) => {
        $(
            ...
            paste! {
                async fn [<$name _zk>](ctx: TestContext) -> Result<()> {
                    // build path to test vector data
                    let workspace_path = std::path::Path::new(&env::var("CARGO_MANIFEST_DIR").unwrap())
                        .ancestors()
                        .nth(1) // Go up 1 level: integration/ -> workspace root
                        ...
                        .to_path_buf();
                    let test_vector_path = workspace_path.join("test_vectors").join(stringify!($name));
                    let proof_file = test_vector_path.join("kat/zk-proof");
                    let vk_file = test_vector_path.join("kat/vk");
                    let public_inputs_file = test_vector_path.join("kat/public_inputs");
                    ...
                    let is_valid = ctx.zk_flavored_verifier_contract()
                    .verify(proof_u8.into(), public_inputs_u8.into(), vk_u8.into())
                    .call()
                    .await?._0;
                ...
                integration_test_async!([<$name _zk>]);
            }
        )*
    };
}
