...
impl Transcript {
    ...
    /// Reads the next `n` elements from the transcript.
    /// Labels were omitted for binary size optimization, altough comments are kept for clarity.
    ...
    /// Reads the next ScalarField from the transcript.
    ...
    /// Reads the next G1Affine from the transcript.
    ...
    /// Reads the next `n` ScalarFields from the transcript.
    ...
    /// Reads the next `SIZE` ScalarFields from the transcript.
    ...
    ///
    /// `lo` is 128 bits and `hi` is 126 bits.
    ...
    ///
    /// This function computes a new challenge for the current round using the previous challenge
    ...
    /// After all the prover messages have been sent, finalize the round by hashing all the data and then
    /// create the number of requested challenges.
    ///
    /// Challenges are generated by iteratively hashing over the previous challenge, using
    ...
    /// Get the next `num_challenges` challenges.
    ///
    /// Labels were omitted for binary size optimization, altough comments are kept for clarity.
