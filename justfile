...
# Build the ultrahonk library for wasm target with only-arithmetic-backend feature.
build-ultrahonk:
  cargo build -p ultrahonk --release --features ark-ec/only-arithmetic-backend --target wasm32-unknown-unknown

# Build the contracts in wasm for stylus. "Contract" is the feature flag within the contracts crate.
...
# Build and package the cli binary for macos
...
# Profile the contracts using twiggy. "Contract" is the feature flag within the contracts crate.
...
# Check the contracts for deployment using stylus. "Contract" is the feature flag within the contracts crate.
...
# Deploy the contracts using stylus. "Contract" is the feature flag within the contracts crate. Pass constructor_signature and constructor_args to deploy with a constructor.
...
# Run the ultrahonk integration tests.
...
# Run the contracts integration tests.
test-contracts-integration:
  cargo run -p integration -- --rpc-url {{rpc_url}} --priv-key {{private_key}}

# Verify a proof from the test vectors with cast. Intended for global verifier only.
...
# Get the verifier addresses for the global verifier.
...
# Spin up the nitro testnode.
nitro-testnode-up:
  ./scripts/nitro-testnode.sh --detach

# Quit the nitro testnode.
nitro-testnode-down:
...
# Format the code and fix the errors.
fmt-fix:
...
# Clean the cli binary for macos
clean-cli-macos:
